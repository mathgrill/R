...
from
    (select coalesce(race_new, 'NA') as race_new, sum(case when intervention = '2' then 1 else 0 end) cnt2,
...
Plotly.newPlot('chart1', data, layout);
    ...
    def db_num_var(var):
        connection = DM.get_conn() 
        sql = "select intervention, length_of_stay::numeric as my_var from public.vw_exp1 where length_of_stay is not null"
...
from (
select intervention, length_of_stay from public.vw_exp1 where length_of_stay is not null ) t
